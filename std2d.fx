...
float4 PS_Std2D(VS_OUT _in) : SV_Target
{
    ...
    if (g_btex_0)//이게 0이라해서 안돌아가지만
    {
        if (bAnimUse)
        {
            ...
            {
                vOutColor = float4(1.f, 1.f, 0.f, 0.f); //여분 컬러 노란색
                //discard;
            }
        }
        else
    ...
    if (0.f == vOutColor.a)//alpha가 0이면
    {
        //return float4(1.f, 0.f, 1.f, 0.f); // 애니메이션 여백 마젠타
        discard;
    }
...
{
    float3 vLocalPos : POSITION;
    float2 vUV : TEXCOORD;//texture좌표(uv)를 RGBA로 해석
};
...
float4 PS_Gauge(G_OUT _in) : SV_Target
{
   ...
   //_in.vUV.y = _in.vUV.y*GaugeRatio ;//-로 하면 아래로 밀리는 느낌으로됨
    if (g_btex_0)
    {
    ...
    if (g_btex_0)//이게 0이라해서 안돌아가지만
    {
        if (bAnimUse)
        {
            ...
            if (LeftTop.x < vUV.x && vUV.x < LeftTop.x + Slice.x
                && LeftTop.y < vUV.y && vUV.y < LeftTop.y + Slice.y)
            {
                vOutColor = g_tex_0.Sample(g_sam_0, vUV);//여기에는 바인딩이 되어있었다
            }
            else
            {
                vOutColor = float4(1.f, 1.f, 0.f, 0.f);                         //여분 컬러 노란색
