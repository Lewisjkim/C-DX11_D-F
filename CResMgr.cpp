...
void CResMgr::CreateDefaultGraphicsShader()
{
	...
	pShader->CreateVertexShader(L"shader\\std2d.fx", "VS_Fade2D");
	pShader->CreatePixelShader(L"shader\\std2d.fx", "PS_Fade2D"); 
	...
	//Test for gauge
	pShader = new CGraphicsShader;
	pShader->SetKey(L"GaugeShader");
	pShader->CreateVertexShader(L"shader\\std2d.fx", "VS_Gauge");
	pShader->CreatePixelShader(L"shader\\std2d.fx", "PS_Gauge");
	...
	pShader->SetRSType(RS_TYPE::CULL_NONE);
	pShader->SetDSType(DS_TYPE::LESS);
	pShader->SetBSType(BS_TYPE::ALPHA_BLEND);//원래는 Mask였음

	pShader->SetDomain(SHADER_DOMAIN::DOMAIN_POSTPROCESS);//DOMAIN_TRANSPARENT
	// Param
	...
	pShader->SetDomain(SHADER_DOMAIN::DOMAIN_MASK);//DOMAIN_MASK
	...
	pShader->SetRSType(RS_TYPE::CULL_NONE);
	pShader->SetDSType(DS_TYPE::NO_TEST_NO_WRITE);//greater해야지 안보인다
	pShader->SetBSType(BS_TYPE::DEFAULT);
...
void CResMgr::CreateDefaultComputeShader()
{
	...
	pCS->CreateComputeShader(L"shader\\setcolor.fx", "CS_SetColor");//노란 십자가를 출력하는 쉐이더
	AddRes(pCS->GetKey(), pCS);

	// Particle Update 쉐이더
	pCS = new CParticleUpdateShader(128, 1, 1);
	pCS->SetKey(L"ParticleUpdateCS");
	pCS->CreateComputeShader(L"shader\\particle_update.fx", "CS_ParticleUpdate");//particle update
	AddRes(pCS->GetKey(), pCS);
}
...
void CResMgr::CreateDefaultMaterial()
{
	...
	//Test for gauge - GaugeShader
	pMtrl = new CMaterial(true);
	pMtrl->SetShader(FindRes<CGraphicsShader>(L"GaugeShader"));
	AddRes(L"GaugeMtrl", pMtrl);
	// Test Material
	pMtrl = new CMaterial(true);
	...
	// Std2DAnim Material == tex와 변수들을 공유하지 않게 
	pMtrl = new CMaterial(true);
	pMtrl->SetShader(FindRes<CGraphicsShader>(L"Std2DShader"));
	...
	pMtrl = new CMaterial(true); // 기본인 false여서 
...
void CResMgr::CreateDefaultPrefab()
{
	...
	//pMissile->SetName(L"MisslePrefab");
	...
	////SpawnGameObject(pMissile, Vec3(50.f, 50.f, 50.f), 18);
