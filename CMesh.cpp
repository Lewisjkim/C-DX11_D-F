...
{
	m_VtxCount = _iVtxCount; //test.cpp에 있는 vertex count들은 휘발성이기에 옮겨다 둔거다
	m_IdxCount = _IdxCount;

	//system memory 데이터 복사
	m_pVtxSys = new Vtx[m_VtxCount];
	memcpy(m_pVtxSys, _VtxSysMem, sizeof(Vtx) * m_VtxCount);

	m_pIdxSys = new Vtx[m_IdxCount];
...
void CMesh::UpdateData()
{
	...
	CONTEXT->IASetVertexBuffers(0, 1, m_VB.GetAddressOf(), &iStride, &iOffset);//Resmgr에서 mesh생성할 때 vb도 같이 만듬
