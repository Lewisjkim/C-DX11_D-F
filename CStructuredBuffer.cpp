	...
	, SB_TYPE _Type, bool _bSysAccess, void* _pSysMem)
{
	...
	// 16바이트 단위 메모리 정렬 = particle 구조체를 16byte단위로 구성해서
	...
	if (nullptr == _pSysMem)// sysmem이 없다면
	{
		if (FAILED(DEVICE->CreateBuffer(&m_tDesc, nullptr, m_SB.GetAddressOf())))// 버퍼를 만들고
		{
			assert(nullptr);
		}
	}
	else//sysmem이 있다면
	{
		...
		tSubData.pSysMem = _pSysMem; // 버퍼 데이터 초기화한 후

		HRESULT hr = DEVICE->CreateBuffer(&m_tDesc, &tSubData, m_SB.GetAddressOf());//버퍼를 만들어라
		if (hr)
		{
			assert(nullptr);
		}
...
void CStructuredBuffer::SetData(void* _pSrc, UINT _iSize) //CPU->CPUwriteBuffer->Main Buffer(GPU)
{
	if (nullptr == _pSrc)
		return;
...
void CStructuredBuffer::GetData(void* _pDst)//MainBuffer(GPU)->CPUreadBuffer->CPU
{
...
void CStructuredBuffer::Clear()//렌더
{
