...
{
private:
    CComponent*                    m_arrCom[(UINT)COMPONENT_TYPE::END];//component arr
...
public:
    ...
    void SetLifeSpan(float _fTime) //수명 계산
    {
        if (m_bLifeSpan)
            return;
...
public:
    CGameObject();
    CGameObject(const CGameObject& _Other);//원본이 인자로 들어온다
    ~CGameObject();
