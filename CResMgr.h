...
public:
    ...
    Ptr<CTexture> CreateTexture(const wstring& _strKey, UINT _Width, UINT _Height
        , DXGI_FORMAT _pixelformat, UINT _BindFlag, D3D11_USAGE _Usage);// resmgr에서 등록하고 관리하게 여기서 만듬

    Ptr<CTexture> CreateTexture(const wstring& _strKey, ComPtr<ID3D11Texture2D> _Tex2D);// resmgr에서 등록하고 관리하게 여기서 만듬
...
public:
    ...
    template<typename T>
    Ptr<T> LoadAtlas(const wstring& _strKey, const wstring& _strRelativePath);
...
template<typename T>
inline Ptr<T> CResMgr::LoadAtlas(const wstring& _strKey, const wstring& _strRelativePath)
{
    Ptr<CRes> pRes = FindRes<T>(_strKey).Get();
