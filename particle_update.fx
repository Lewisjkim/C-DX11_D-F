...
[numthreads(128, 1, 1)]
void CS_ParticleUpdate(int3 _ID : SV_DispatchThreadID) //디스패치 호출 내에서 그룹 차원당 전역 스레드 오프셋을 정의합니다. 컴퓨팅 셰이더에 대한 입력으로 사용할 수 있습니다. (읽기 전용)
{
    // 스레드 ID 가 파티클버퍼 최대 수를 넘긴경우 
    ...
    if (SpawnModule)
    {
        ...
        if (particle.Active == 0) 
        {
            ...
            while (0 < SpawnCount)
            {
                ...
                int outvalue = 0;
                InterlockedCompareExchange(SpawnCount, orgvalue, SpawnCount - 1, outvalue); // 상호 잠긴 비교 및 교환을 수행합니다.
                
            
                if (orgvalue == outvalue) 
                {
                    particle.Active = 1; // 활성화 시켜주고
                    ...
                    if (ModuleData.AddVelocity)
                    {
                        ...
                        {
                            float3 vVelocity = normalize(particle.vLocalPos.xyz); //velocity가 아닌 방향
                        ...
                        {
                            float3 fixedDir = normalize(ModuleData.vVelocityDir.xyz);
                            particle.vVelocity.xyz = fixedDir * ModuleData.Speed;
