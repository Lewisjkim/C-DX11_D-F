...
void TreeNode::render_update()
{
    ...
    if (ImGui::TreeNodeEx(strFinalName.c_str(), flag))
    {
        ...
        else if (ImGui::IsItemHovered() && ImGui::IsMouseReleased(ImGuiMouseButton_::ImGuiMouseButton_Right))
        {
            ...
            m_Owner->CreatePrefab(this);
        }
        else if (0 != m_Owner->m_dwPrevSelected && m_Owner->m_dwPrevSelected == m_Data)
        {
           // m_Owner->m_dwPrevSelected = 0;
            //m_Owner->ChangeNameNode(this);
        }
        
        //마우스가 위에있으면 안내문
        if (ImGui::IsItemHovered() && !ImGui::IsMouseDown(ImGuiMouseButton_::ImGuiMouseButton_Left))
        {
            if ("OutlinerTree" == m_Owner->GetName())
            {
                if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayShort))
                {
                    ImGui::BeginTooltip();
                    ImGui::PushTextWrapPos(ImGui::GetFontSize() * 35.0f);
                    ImGui::TextUnformatted("Right click to make Prefab");
                    ImGui::PopTextWrapPos();
                    ImGui::EndTooltip();
                }
            }
            if ("ContentTree" == m_Owner->GetName())
            {
                if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayShort))
                {
                    ImGui::BeginTooltip();
                    ImGui::PushTextWrapPos(ImGui::GetFontSize() * 35.0f);
                    ImGui::TextUnformatted("Left click to see Info");
                    ImGui::PopTextWrapPos();
                    ImGui::EndTooltip();
                }
            }
            
            //ImGui::Text("Right click to make Prefab");
            //ImGui::TextWrapped("Right click to make Prefab");
        }
...
void TreeUI::CreatePrefab(TreeNode* _Node)
{
    //우클릭 노드가 아니라면 nullptr로 해두고 return
    ...
    if (m_SelectedNode)
    {
        ...
        if (m_SelectInst && m_CreatePrefabFunc)
        {
            (m_SelectInst->*m_CreatePrefabFunc)((DWORD_PTR)m_SelectedNode); // 우클릭한 노드를 인자로 놈겨줄거다
        }
    }
}

void TreeUI::ChangeNameNode(TreeNode* _Node)
{
   /* if (m_RbtDownNode != _Node)
    {
        m_RbtDownNode = nullptr;
   ...
   // m_RbtDownNode = nullptr;
    ...
    if (m_SelectedNode)
    {
        ...
        if (m_SelectInst && m_ChangeNameFunc)
        {
            (m_SelectInst->*m_ChangeNameFunc)((DWORD_PTR)m_ChangeNode); // 우클릭한 노드를 인자로 놈겨줄거다
        }
    }
}
