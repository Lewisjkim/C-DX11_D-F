...
void ListUI::finaltick() // list 버튼을 눌렀을 때 들어온다
{
    ...
    if (ImGui::BeginListBox("##list", ListUIContentSize))
    {
        for (int i = 0; i < m_vecStrData.size(); i++)
        {
            ...
            if (ImGui::IsItemHovered() &&
                ImGui::IsMouseDoubleClicked(ImGuiMouseButton_::ImGuiMouseButton_Left))//리스트 아이템에 올라가 더블 클릭시
            {
                SetActive(false);
                ImGui::SetWindowFocus(nullptr);
...
{
    Clear(); // 한번 클리어 해 주어서 전에 list를 제거
    SetName(_strName); // 인자로 받은 strName을 이름으로 설정
    ...
    RECT rt = {};
    //GetClientRect(hWnd, &rt); // main window내에서의 사이즈와 시작 점
