...
{
    float2 vUV = float2(_NomalizedThreadID, 0.5f);//id위치, y축 가운데
    
    vUV.x += g_AccTime * 0.5f; //누적시간 *0.5 를 계속 x값아 넣어 주어 이동하게 끔
    
    // sin 그래프로 텍스쳐의 샘플링 위치 UV 를 계산
    vUV.y -= (sin((_NomalizedThreadID - (g_AccTime /*그래프 우측 이동 속도*/)) * 2.f * 3.1415926535f * 10.f /*반복주기*/) / 2.f);
    
    if (1.f < vUV.x)//1을 넘어가도
        vUV.x = frac(vUV.x);//소수 부분만 반환 fraction(분수)
    else if (vUV.x < 0.f)
        vUV.x = 1.f + frac(vUV.x);//음수여도 소수점만 반환
