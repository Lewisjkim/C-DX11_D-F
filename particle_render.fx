...
struct VS_IN
{
    ...
    uint iInstID : SV_InstanceID; //런타임에 의해 자동으로 생성된 인스턴스별 식별자입니
};
...
struct GS_OUT
{
    ...
    float2 vUV : TEXCOORD; //질감 좌표
    uint iInstID : SV_InstanceID;
};
//graphic shader - create geometry shader - 파티클 모양
[maxvertexcount(6)] //꼭짓점 수
void GS_ParticleRender(point VS_OUT _in[1], inout TriangleStream<GS_OUT> _outstream) //VS_OUT에서 1개 받아오고, 삼각형 프리미티브형식 gs_out을 받아온다 https://learn.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-hlsl-so-type
{
    uint id = _in[0].iInstID;// 받아온 1개의 vertex의 id를 설정
  
    if (0 == ParticleBuffer[id].Active) //파티클 버퍼가 활성화 되었다면
        return;
 ...
 // graphic shader - createpixel shader - 색 관련~
